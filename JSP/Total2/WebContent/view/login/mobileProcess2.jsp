...
<%
	...
	//체크인 요청
	// 체크인 이전에 tag가 위조인지 아닌지 확인
	lecroom_id = request.getParameter("lecroom_id");
	int id_tag_new = requestCheckBiz.isInHere(lecroom_id, width,
			height, tag_id);
	if (id_tag_new < 10000) {//결과가 10000 미만이라면 복제태그가 아님.
		// 체크인 Biz
		lecture_id = requestCheckBiz.getLecture_id(lecroom_id);

		int class_count = requestCheckBiz.answerPossible(student_id,
				lecture_id);
		...
		if (class_count < 0) {
			...
			result = requestCheckBiz.CheckIn(student_id, lecture_id,
					class_count, width, height);
			String _id_tag_new = Integer.toString(id_tag_new);
			if (result) {
				//출석체크 성공
				session.setAttribute("mobile", _id_tag_new);
