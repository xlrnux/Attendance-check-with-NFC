...
<%
	...
	System.out.println("setInfo2.jsp:lecture_id: " + subj_id);
	System.out.println("setInfo2.jsp:lecroom_id: " + lecroom_id);
	Room room = new Room();
	...
	if (lecture.getToday() == 2) {
		...
		System.out.println("setInfo2.jsp:lecture_name: "
				+ lecture.getLecture_name());
		session.setAttribute("lecture_name", lecture.getLecture_name());
		if (lecture.getToday() == 1) {
			session.setAttribute("class_count",
					new Integer(lecture.getClass_count()));
			lectureSettingBiz.lectureOn(subj_id, lecroom_id);
		} else {
			session.setAttribute("class_count",
					new Integer(lecture.getClass_count() + 1));
			lectureSettingBiz.lectureOn(subj_id, lecroom_id,
					lecture.getClass_count() + 1);
			// 여기부터 출석부 목록 복사
			UpdateCheckListBiz updateCheckListBiz = new UpdateCheckListBiz();
			updateCheckListBiz.updateCheckList(subj_id,
					lecture.getClass_count() + 1);
		}
		...
		System.out.println("lecture_id:setInfo2.jsp: " + subj_id);
		System.out.println("lecroom_id:setInfo2.jsp: " + lecroom_id);
